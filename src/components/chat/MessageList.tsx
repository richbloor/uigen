...
function getToolLabel(toolName: string, args: Record<string, unknown>, isDone: boolean): string {
  ...
  if (toolName === "str_replace_editor") {
    switch (args?.command) {
      ...
      case "view":    return isDone ? `Viewed ${filename}` : `Viewing ${filename}`;
    }
  }
                              ...
                              <div key={partIndex} className="inline-flex items-center gap-2 mt-2 px-3 py-1.5 bg-neutral-50 rounded-lg text-xs font-mono border border-neutral-200">
                                {tool.state === "result" && tool.result ? (
                                  <>
                                    ...
                                    <span className="text-neutral-700">{getToolLabel(tool.toolName, tool.args as Record<string, unknown>, true)}</span>
                                  </>
                                ) : (
                                  <>
                                    <Loader2 className="w-3 h-3 animate-spin text-blue-600" />
                                    <span className="text-neutral-700">{getToolLabel(tool.toolName, tool.args as Record<string, unknown>, false)}</span>
                                  </>
                                )}
                              </div>
                            );
